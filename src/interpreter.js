...
function evaluate_object_expression(input_text,stmt,env) {
	...
	var object = make_object(evaluated_pairs,{});
	add_object_object(object);
	return object;
}
...
function extend_environment(vars,vals,base_env) {
	...
	if (var_length === val_length) {
		...
		if(!is_empty_environment(base_env)){
			define_variable("_contain", pair(new_frame, head(base_env)["_contain"]), base_env);
		}
		var new_env = enclose_by(new_frame,base_env);
		define_variable("_contain", [], new_env);
		return new_env;
...
function evaluate(input_text,stmt,env) {
	if ((new Date()).getTime() > expires) {
		...
		if (fun === get_frames){
			return pair(global_frame_info(), map(get_frames, first_frame(the_global_environment)["_contain"]));
		} else if (fun === debug_break) {
...
function add_function_objcet(function_value){
	...
	obj.body = get_body_text(function_value_source_text(function_value)[0]);
	obj.index = all_functions.length;
	all_functions.push(pair(function_value, obj));
}

function add_object_object(object_value){
	all_objcets.push(object_value);
}
...
function get_object_index(object_value){
	for (var i in all_objcets){
		if(all_objcets[i] === object_value){
			return i;
		}
	}
}
...
function object_object(value){
	var obj = {};
	var table = [];
	obj.tag = "object";
	obj.index = get_object_index(value);
	get_object_info(value, table);
	obj.info = table;
	return obj;
}

function add_key_value_pair(key, value, table){
	if(key !== "this"){
		...
		if(is_compound_function_value(value)){
			...
			object = pair_object(value);
		} else if(is_object(value) && key !== "this"){
			object = object_object(value);
		} 
		table.push(pair(key, object));
	}
}

function get_object_info(object_value, table){
	$.each(object_value, function(key, value){
		add_key_value_pair(key, value, table);
	});
}
...
function frame_info(frame){
	...
	table.push(pair("local_functions", local_functions));
...
function global_frame_info(){
	...
	$.each(first_frame(the_global_environment), function(key, value){
...
function get_frames(frame){
	return pair(frame_info(frame), map(get_frames, frame["_contain"]));
}
