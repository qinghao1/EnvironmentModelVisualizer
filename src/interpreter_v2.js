...
function evaluate_assignment(input_text,stmt,env) {	
	return set_variable_value(assignment_variable(stmt),
		evaluate(input_text,assignment_value(stmt),env),
		env);
}
...
var primitive_functions = 
	...
	//Environmental model functions
	pair("get_frames", get_frames),

	//debug functions
	pair("debug", debug_break),
	pair("debug_resume", debug_resume),
...
var primitive_function_names = map(head, primitive_functions);
var primitive_function_objects = map(
		function(f) {
...
var primitive_value_names = map(head, primitive_value_pairs);
var primitive_values = map(tail, primitive_value_pairs);

var all_libraries = list("JSON", "Math");
...
function has_value(values, value){
	if(is_empty_list(values)){
		return false;
	} else if(head(values) === value){
		return true;
	} else {
		return has_value(tail(values), value);
	}
}

function is_primitive_variable(variable){
	return has_value(primitive_value_names, variable);
}

function is_library(variable){
	return has_value(all_libraries, variable);
}

function get_body_text(text){
	var index = text.indexOf("{");
	return text.substr(index);
}

function get_frame_index(frame){
	return frame["__viz_index"];
}

//When constructing new frame/function/empty_list, add the new value to corresponding array in order to keep track
function add_frame(frame, base_env){
	frame["__viz_index"] = all_frames.length;
	if(!is_empty_list(base_env)){
		frame["__viz_enclosing"] = get_frame_index(first_frame(base_env));
	} else {
		frame["__viz_enclosing"] = undefined;
	}
	all_frames.push(frame);
	all_tables.push([]);
}

function add_function_objcet(function_value){
	var obj = {};
	obj["parameters"] = list_to_array(function_value_parameters(function_value));
	obj.name = function_value.name;
	obj.body = get_body_text(function_value.text);
	obj.index = all_functions.length;
	all_functions.push(pair(function_value, obj));
}

function add_empty_list(empty_list){
	all_empty_lists.push(empty_list);
}

/**
* Obtain the index of input functions
*/
function get_function_index(function_value){
	for (var i in all_functions){
		if(head(all_functions[i]) === function_value){
			return tail(all_functions[i]).index;
		}
	}
}

/**
* Obtain the index of input empty_list
*/
function get_empty_list_index(empty_list){
	for (var i in all_empty_lists){
		if(all_empty_lists[i] === empty_list){
			return i;
		}
	}
}

//A couple of functions making value of different data type to tagged objects in consistent format
function primitive_object(value){
	var obj = {};
	obj.tag = "primitive";
	obj.value = value;
	return obj;
...
function empty_list_object(value){
	var obj = {};
	obj.tag = "empty_list";
	obj.index = get_empty_list_index(value);
	return obj;
}

function pair_list_object(value){
	var obj = {};
	obj.tag = "pair";
	obj.value = value;
	return obj;
...
function function_object(value){
	var obj = {};
	obj.tag = "function";
	obj.index = get_function_index(value);
	obj["defined_in"] = get_frame_index(first_frame(function_value_environment(value)));
	return obj;
}

function undefined_object(value){
	var obj = {};
	obj.tag = "undefined";
	return obj;
}
...
/**
* Add a new row using the given key and value to the given table
*/
function add_key_value_pair(key, value, table, row){
	var object = undefined;
	if(is_compound_function_value(value)){
		object = function_object(value);
	} else if(is_number(value) || is_string(value) || is_boolean(value)){
		object = primitive_object(value);
	} else if(is_empty_list(value)){
		object = empty_list_object(value);
	} else if(is_pair(value)){
		object = pair_list_object(value);
	} else {
		object = undefined_object(value);
	} 
	table[row] = pair(key, object);
}

/**
* Get the infomation of the frame specified by the input index and fill the corresponding table
*/
function get_frame_info(index){
	var frame = all_frames[index];
	var table = all_tables[index];

	table[0] = pair("__viz_index", frame["__viz_index"]);
	table[1] = pair("__viz_enclosing", frame["__viz_enclosing"]);

	//locally defined functions
	var local_functions = [];
	for(var i=0; i<all_functions.length; i++){
		var fun = head(all_functions[i]);
		if(first_frame(function_value_environment(fun)) === frame){
			local_functions.push(tail(all_functions[i]));
		}
	}
	table[2] = pair("local_functions", local_functions);

	//go into the frame and use the infomation to fill the table
	var row = 3;
	$.each(frame, function(key, value){
		if(key !== "this" && key !== "__viz_index" && key !== "__viz_enclosing"){
			add_key_value_pair(key, value, table, row);
			row++;
		}
	});
}
...
/**
* Get infomation from the global frame and fill the corresponding table
*/
function get_global_frame_info(){
	var frame = all_frames[0];
	var table = all_tables[0];

	table[0] = pair("__viz_index", frame["__viz_index"]);
	table[1] = pair("__viz_enclosing", frame["__viz_enclosing"]);

	//variables to store special data
	var primitive_functions = [];
	...
	for(var i=0;i<all_functions.length;i++){
		var fun = head(all_functions[i]);
		if(first_frame(function_value_environment(fun)) === first_frame(the_global_environment)){
			local_functions.push(tail(all_functions[i]));
		}
	}
	table[2] = pair("local_functions", local_functions);
	...
	//sort infomation and fill the table
	var row = 3;
	$.each(frame, function(key, value){
		if(is_primitive_function(value)){
			primitive_functions.push(pair(key, value));
		} else if(is_primitive_variable(key)){
			primitive_values.push(pair(key, value));
		} else if(is_library(key)){
			libraries.push(pair(key, value));
		} else if(key !== "__viz_index" && key !== "__viz_enclosing"){
			add_key_value_pair(key, value, table, row);
			row++;
		}
	});
...
/**
* Returns an array of tables, each of them represents a specific frame and contains all the 
* key-value pairs in that frame.
*/
function get_frames(){
...
var the_global_environment = (function() {
	var initial_env = extend_environment(append(primitive_function_names, primitive_value_names),
		append(primitive_function_objects, primitive_values),
