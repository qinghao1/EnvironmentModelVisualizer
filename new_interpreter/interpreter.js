        ...
        (function(){
            ...
            function* evaluate_unary_expression(input_text,stmt,env) {
            ...
            function* evaluate_binary_expression(input_text,stmt,env) {
                ...
                if (is_true(predicate)) {
                    ...
                    if (check_generator(consequent)) {
                        ...
                        while (!next.done) {
                            ...
                            next = consequent.next();            
                        }
                        if (next.done) consequent = next.value;            
                    }   
                    if (check_generator(consequent)) {
                        yield;
                        let next = consequent.next();
                    ...
                    if (check_generator(alternative)) {
                        ...
                        if (next.done) alternative = next.value;            
                    }   
                    if (check_generator(alternative)) {
                        yield;
                        let next = alternative.next();
            ...
            function* evaluate_while_statement(input_text,stmt, env) {
                ...
                if (check_generator(condition)) {
                    ...
                    if (next.done) condition = next.value;            
                }   
                if (check_generator(condition)) {
                    yield;
                    let next = condition.next();
            ...
            function* evaluate_for_update(input_text, stmt, env) {
                var temp = evaluate(input_text,for_finaliser(stmt),env);
                if (check_generator(temp)) {
                        let next = temp.next();
                        while (!next.done) {
                            if (inter_breakpoints.has(inter_current_line)) {
                                break;
            ...
            function* evaluate_for_statement(input_text,stmt, env) {
                let result = undefined;
                let initial = evaluate(input_text,for_initialiser(stmt), env);
                if (check_generator(initial)) {
                    initial = evaluate_generator(initial);
                }
                for (initial;
                    is_true(evaluate_generator(evaluate(input_text, for_predicate(stmt), env)));
                    evaluate_generator(evaluate_for_update(input_text,stmt,env))) {
                    yield;
            ...
            function declarations_to_list(declarations) {
                let s = [];
                let i = 0;
                while(typeof(declarations[i]) !== 'undefined') {
                    i++;
                }
                for( j = i - 1; j >= 0; j--) {
                    s = pair(declarations[j].id.name,s);
                }
                return s;
            }

            function take_function_variables(body) {
                let result = [];
                for (var key in body) {
                    if (body.hasOwnProperty(key)) {
                        if (key === 'declarations') {
                            result = append(result, declarations_to_list(body[key]));
                        } else if (typeof(body[key]) === 'object' && body[key] != null) {
                            result = append(result,take_function_variables(body[key], result));
                        } 
                    }
                }
                return result;
            ...
            function make_function_value(input_text,name,parameters,body,location,env) {
                ...
                let temp = params_to_list(parameters);
                result.variables = take_function_variables(body);
            ...
            function function_value_variables(value) {
            ...
            function evaluate_construction_statement(input_text,stmt, env) {
                ...
                if (check_generator(extraResult)) {
                    let next = extraResult.next();
                    while (!next.done) {
                        if (inter_breakpoints.has(inter_current_line)) {
                            break;
                        } 
                        next = extraResult.next();            
                    }
                    if (next.done) extraResult = next.value;            
                }   
                if (check_generator(extraResult)) {
                    yield;
                    let next = extraResult.next();
                    yield;
                    while (!next.done) {
                        ynext = extraResult.next();
                        yield;
                    }
                    extraResult = next.value;
                }    
            ...
            function is_application(stmt) {
                return (is_tagged_object(stmt,"CallExpression") && is_tagged_object(stmt.callee, "Identifier")) || (is_tagged_object(stmt,"CallExpression") && is_tagged_object(stmt.callee, "CallExpression"));
            }

            function* evaluate_application(input_text,stmt,env) {
                 let fun = evaluate(input_text,stmt.callee,env);
                    if (check_generator(fun)) {
                        let next = fun.next();
                        while (!next.done) {
                            if (inter_breakpoints.has(inter_current_line)) {
                                break;
                    ...
                    if (check_generator(fun)) {
                     ...
                     if (fun === debug_break) {
                         ...
                         let val = apply(fun, args, context);  
            ...
            function* evaluate_object_method_application(input_text,stmt,env) {
            ...
            function* evaluate_return_statement(input_text,stmt,env) {
            ...
            function evaluate(input_text,stmt,env) {
                ...
                if ((new Date()).getTime() > expires) {
                    ...
                    return evaluate_unary_expression(input_text,stmt,env);
                } else if(is_binary_expression(stmt)) {
                    return evaluate_binary_expression(input_text,stmt,env);
                } else if (is_application(stmt)) {
                    return evaluate_application(input_text,stmt,env);
                } else if (is_object_method_application(stmt)) {
                    return evaluate_object_method_application(input_text,stmt,env);
                    ...
                    return evaluate_return_statement(input_text,stmt,env);
